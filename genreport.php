...
function getuserlist()
{
  ...
  $res = mysql_query("SELECT user_id FROM user_info WHERE (type='C') ORDER BY grp, user_id");
...
//  $res = mysql_query("SELECT prob_id FROM prob_info WHERE (avail='Y')");
  $res = mysql_query("SELECT  prob_info.prob_id  FROM `lessons`,lesson_prob,prob_info WHERE lessons.active=1 and lessons.id=lesson_prob.lesson_id and lesson_prob.active=1 and prob_info.prob_id=lesson_prob.prob_id");
  $row=mysql_num_rows($res);
  for($i=0; $i<$row; $i++) {
...
function genreport_user($uid)
{
  ...
  echo "-------------------------------------------------------<br>\n";
  $res = mysql_query("SELECT user_id, name, grp, scid FROM user_info " .
                     "WHERE (user_id='$uid') and type='C'");
  $row=mysql_num_rows($res);
  echo "user: " . $uid . " : " . mysql_result($res,0,'name') . "<br>\n";
  echo "school: " . getschool(mysql_result($res,0,'user_id')) . "<br>\n";
